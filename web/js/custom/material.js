...
var handleTitle = function(tab, navigation, index) {
    ...
    if (current == 1) {
        $('#form_wizard_1').find('.materialZuruck').hide();
    } else {
        $('#form_wizard_1').find('.materialZuruck').show();
    }

    if (current >= total) {
        $('#form_wizard_1').find('.materialNext').hide();
        $('#form_wizard_1').find('.materialSubmit').show();
    } else {
        $('#form_wizard_1').find('.materialNext').show();
        $('#form_wizard_1').find('.materialSubmit').hide();
    }
   
}

$('#form_wizard_1').bootstrapWizard({
    'nextSelector': '.materialNext',
    'previousSelector': '.materialZuruck',
...
$('#form_wizard_1').find('.materialZuruck,.materialSubmit').hide();
...
function displayOverView()
{
    ...
    $('#materialOverView_pz_alt1').html($('#normbezeichnung').val());
    $('#materialOverView_werkstoffnummer').html($('#werkstoffnummer').val());

    $('#materialOverView_trivialname1').html($('#trivialname1').val());
    $('#materialOverView_trivialname2').html($('#trivialname2').val());
    $('#materialOverView_trivialname3').html($('#trivialname3').val());
    $('#materialOverView_werkstoffzustand').html($('#werkstoffzustand').val());
    $('#materialOverView_schmelze').html($('#schmelze').val());
    $('#materialOverView_weld_mat').html(($('#weld_mat').is(":checked"))? "X" : "");
    $('#materialOverView_pz_weld2').html($('#pz_weld2').val());
    $('#materialOverView_pz_weld3').html($('#pz_weld3').val());


}

$("#weld_mat").change(function(){

    if($(this).is(":checked"))
    {
        $("#pz_weld2,#pz_weld3").prop("disabled",false);
    }

    else
    {
        $("#pz_weld2,#pz_weld3").val("").prop("disabled",true);
    }
})
...
$('#eingabePrufzeichen').keyup(function(){

    var val = $.trim($(this).val());
    this.value = this.value.replace(/[iIloOöÖüÜxX0-9]/g, '');
    val = val.charAt(0).toLowerCase() + val.slice(1).toUpperCase();
    ...
    $('body').on('change','#freePrufzeichen', function() {
        $('#eingabePrufzeichen').val(this.value);
    });
...
$("input[name=matbtn]").click(function() {
...
$(".materialSubmit").click(function(){
    ...
    var dataString = $("#materialForm").serialize();
                
    $.ajax({
            type: "POST",
            url: "save-neue-material",
            data: dataString,
            async: true,
            dataType: 'json',
            beforeSend:function(){
